...
# MAGIC %pip install "unstructured[pdf,docx]==0.10.30" llama-index==0.9.3
...
embeddings = deploy_client.predict(endpoint=EMBEDDING_ENDPOINT, inputs={"inputs": ["What is Apache Spark?"]})
pprint(embeddings)
...
def get_embedding(contents: pd.Series) -> pd.Series:
    ...
    def get_embeddings(batch):
        ...
        response = deploy_client.predict(endpoint=EMBEDDING_ENDPOINT, inputs={"inputs": batch})
...
dbutils.fs.rm(f'dbfs:{volume_folder}/checkpoints/docs_chunks', True)
...
# MAGIC %sql ALTER TABLE databricks_demos.rag_chatbot_c5379186.databricks_pdf_documentation SET TBLPROPERTIES (delta.enableChangeDataFeed = true)
...
response = deploy_client.predict(endpoint=EMBEDDING_ENDPOINT, inputs={"inputs": [question]})
